...
export default function UpdateAddress({ match, setCartItems }) {
  ...
  const getData = () => {
    axios
      .get(`/address/${id}`)
      .then((res) => {
        const { full_name, address, zip_code, region, contact_num } =
          res.data.address
  ...
  const submitHandler = (e) => {
    ...
    if (fullname && ad1 && ad2 && zip && region && pno) {
      axios
        .put(`/address/edit/${id}`, inputValue)
        .then((res) => {
          if (res.status === 204) {
  ...
  return (
    <>
      ...
      <form className="cstm-frm1" onSubmit={submitHandler}>
        <h1 className="centered">UPDATE ADDRESS</h1>
